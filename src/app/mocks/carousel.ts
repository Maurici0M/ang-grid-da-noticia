import { CarouselData } from '../interfaces/carousel/i-carousel';

export const MOCK_CAROUSEL: CarouselData = {
  card: [
    {
      title: 'Pesquisa (Pós-Graduação)',
      description:
        'Destinada a graduados que desejam realizar pesquisas, mestrado ou doutorado no Japão. Inclui curso de japonês inicial.',
      srcImg:
        'https://images.unsplash.com/photo-1528164344705-47542687000d?q=80&w=1000',
      altImg: '',
      link: '',
    },
    {
      title: 'Graduação (Gakubu)',
      description:
        'Bolsa completa para cursar a faculdade inteira no Japão (5 a 7 anos). Exige alto desempenho em exames de exatas ou humanas.',
      srcImg:
        'https://images.unsplash.com/photo-1540959733332-eab4deabeeaf?q=80&w=1000',
      altImg: '',
      link: '',
    },
    {
      title: 'Escola Técnica (Kosen)',
      description:
        'Focada em áreas de engenharia e tecnologia. Oferece formação prática em colégios tecnológicos japoneses de alto nível.',
      srcImg:
        'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?q=80&w=1000',
      altImg: '',
      link: '',
    },
    {
      title: 'Curso Profissionalizante',
      description:
        'Para quem busca especialização em áreas como Design, Gastronomia, Música ou Animação em escolas especializadas (Senshu).',
      srcImg:
        'https://images.unsplash.com/photo-1513506003901-1e6a229e2d15?q=80&w=1000',
      altImg: '',
      link: '',
    },
    {
      title: 'Treinamento de Professores',
      description:
        'Destinada a professores da rede pública que desejam realizar pesquisas sobre educação em universidades japonesas.',
      srcImg:
        'https://images.unsplash.com/photo-1427504494785-3a9ca7044f45?q=80&w=1000',
      altImg: '',
      link: '',
    },
    {
      title: 'Cultura e Língua Japonesa',
      description:
        'Bolsa de um ano para estudantes de Letras (Japonês) aprofundarem seus conhecimentos culturais e linguísticos no país.',
      srcImg:
        'https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?q=80&w=1000',
      altImg: '',
      link: '',
    },
    {
      title: 'Benefícios da Bolsa',
      description:
        'Passagens aéreas, isenção de taxas escolares e uma ajuda de custo mensal que varia entre 117.000 a 145.000 ienes.',
      srcImg:
        'https://images.unsplash.com/photo-1526304640581-d334cdbbf45e?q=80&w=1000',
      altImg: '',
      link: '',
    },
    {
      title: 'Processo de Seleção',
      description:
        'Composto por análise documental, provas escritas (Inglês, Japonês e matérias específicas) e entrevista no consulado.',
      srcImg:
        'https://images.unsplash.com/photo-1506784983877-45594efa4cbe?q=80&w=1000',
      altImg: '',
      link: '',
    },
  ],
};

export const MOCK_HOME: CarouselData = {
  card: [
    {
      title:
        'Discord: O que é e como funciona o app para gamers que virou foco de abusos contra jovens.',
      description:
        'Plataforma ganhou popularidade entre os jovens por transmissões ao vivo de vídeos das partidas, mas virou mecanismo da ação de criminosos para subornar menores.',
      srcImg:
        'https://t.ctcdn.com.br/OBqzxQ888c3zUJEzGrxu4jjioOo=/512x288/smart/filters:format(webp)/i525670.png',
      altImg: 'Logo do aplicativo Discord',
      link: 'https://www.estadao.com.br/link/discord-o-que-e-e-como-funciona-o-app-para-gamers-que-virou-foco-de-abusos-contra-jovens/',
    },
    {
      title:
        'Nasa divulga foto deslumbrante de lagoa no RS diretamente do espaço',
      description:
        'Imagem feita pelo ex-astronauta Jack Fischer a partir da Estação Espacial Internacional (ISS) mostra a beleza da Lagoa dos Barros, no litoral gaúcho.',
      srcImg:
        'https://s2-galileu.glbimg.com/RclyAxzTmJyrstSzCizMTSSTfJs=/0x0:1600x1064/924x0/smart/filters:strip_icc()/i.s3.glbimg.com/v1/AUTH_fde5cd494fb04473a83fa5fd57ad4542/internal_photos/bs/2023/h/V/4RIemKTaqKU1LBkV3fOw/whatsapp-image-2023-07-14-at-10.18.36.jpeg',
      altImg: 'Imagem da lagoa no RS',
      link: 'https://revistagalileu.globo.com/ciencia/espaco/noticia/2023/07/nasa-divulga-foto-deslumbrante-de-lagoa-no-rs-diretamente-do-espaco.ghtml/',
    },
    {
      title: 'Pudim de leite condensado',
      description:
        'Veja como fazer o pudim de leite condensado tradicional, que leva apenas 4 ingredientes: açúcar, leite condensado, leite e ovos. Ana Maria Braga ensina o passo a passo desta sobremesa tipicamente brasileira que vai encantar sua família.',
      srcImg:
        'https://s2-receitas.glbimg.com/z2JiB6nwO_XITDhQksL2uiAhic0=/0x0:984x765/1000x0/smart/filters:strip_icc()/i.s3.glbimg.com/v1/AUTH_1f540e0b94d8437dbbc39d567a1dee68/internal_photos/bs/2020/D/m/r8XAmWTcuRAzTRMBJZ1A/pudim-de-leite-condensado.jpg',
      altImg: 'Imagem de um pudim de leite condensado',
      link: 'https://receitas.globo.com/receitas-da-tv/mais-voce/pudim-de-leite-condensado-4d514bb052e0b252bc00e85a.ghtml',
    },
    {
      title:
        'Bard, o "ChatGPT do Google", é lançado no Brasil; veja como funciona.',
      description:
        'O Bard, ferramenta de inteligência artificial do Google, foi lançado nesta quinta-feira (13) no Brasil. O serviço, que funciona como o ChatGPT, é gratuito e chega ao país pouco mais de três meses após ser lançado nos Estados Unidos e no Reino Unido.',
      srcImg:
        'https://googlediscovery.com/wp-content/uploads/bard-2-800x500.jpg',
      altImg: 'Logo do Google Bard',
      link: 'https://g1.globo.com/tecnologia/noticia/2023/07/13/bard-o-chatgpt-do-google-e-lancado-no-brasil-veja-como-funciona.ghtml',
    },
    {
      title: 'O Legado do Helicóptero Ingenuity Em Marte.',
      description:
        'Em 19 de abril de 2021, um pequeno helicóptero chamado Ingenuity fez história ao realizar o primeiro voo motorizado e controlado em outro planeta.',
      srcImg:
        'https://spacetoday.com.br/wp-content/uploads/2023/05/ingenuity.jpg',
      altImg: 'Imagem do Helicóptero Ingenuity',
      link: 'https://spacetoday.com.br/o-legado-do-helicoptero-ingenuity-em-marte/',
    },
    {
      title:
        "Inspirada em 'Barbie', doceria Pikurrucha's, em SP, anuncia doces e drinques cor-de-rosa.",
      description:
        'Pioneira no conceito do espaço instagramável, a confeitaria decidiu ampliar o ambiente cor-de-rosa que dialoga com a boneca para seu cardápio, oferecendo opções como um café rosa com brigadeiro branco e pipoca doce.',
      srcImg:
        'https://f.i.uol.com.br/fotografia/2023/07/14/168935808464b18f049419d_1689358084_3x2_lg.jpg',
      altImg: "Imagem da doceria Pikurrucha's",
      link: 'https://guia.folha.uol.com.br/restaurantes/2023/07/inspirada-em-barbie-doceria-pikurruchas-em-sp-anuncia-doces-e-drinques-cor-de-rosa.shtml',
    },
    {
      title:
        'Instagram lança Threads, rede social para rivalizar com o Twitter.',
      description:
        'Rede social foi revelada dias após o Twitter anunciar um limite para leitura de posts. Usuários poderão fazer login e encontrar amigos por meio de suas contas no Instagram.',
      srcImg:
        'https://s2-g1.glbimg.com/XvJgv46mktFspeaOXTWubWa87Mg=/0x0:6000x3375/1008x0/smart/filters:strip_icc()/i.s3.glbimg.com/v1/AUTH_59edd422c0c84a879bd37670ae4f538a/internal_photos/bs/2023/y/6/jynisCQqywFngyjBH5Jg/aplicativo-threads.png',
      altImg: 'Imagens da nova rede social: "Threads"',
      link: 'https://g1.globo.com/tecnologia/noticia/2023/07/05/instagram-threads-rival-twitter.ghtml',
    },
    {
      title:
        'Entenda a foto comemorativa do primeiro ano de atividade do James Webb.',
      description:
        'Nova imagem apresenta a região de formação estelar mais próxima da Terra, no complexo de nuvens Rho Ophiuchi.',
      srcImg:
        'https://s2-galileu.glbimg.com/HsNsFP9yQHqpGx87m0WOKyxrIm4=/0x0:1280x1199/924x0/smart/filters:strip_icc()/i.s3.glbimg.com/v1/AUTH_fde5cd494fb04473a83fa5fd57ad4542/internal_photos/bs/2023/p/o/YgiPfLRHOm0m7L3lJquw/weic2316a.jpg',
      altImg: 'Imagem da nebulosa',
      link: 'https://revistagalileu.globo.com/ciencia/espaco/noticia/2023/07/entenda-a-foto-comemorativa-do-primeiro-ano-de-atividade-do-james-webb.ghtml',
    },
    {
      title: 'Tapioca engorda? Conheça mitos e verdades do alimento.',
      description:
        'A tapioca ganhou destaque em muitas receitas e virou parte do cardápio de quem busca uma alimentação mais saudável. Feita da massa da mandioca, ela também é consumida por quem pratica exercícios físicos, já que funciona como pré e pós-treino.',
      srcImg:
        'https://s2-receitas.glbimg.com/LNr5zJu-Fo1pY5TZjk2iCEOS2PE=/0x0:5184x3456/1000x0/smart/filters:strip_icc()/i.s3.glbimg.com/v1/AUTH_1f540e0b94d8437dbbc39d567a1dee68/internal_photos/bs/2022/B/J/TwmuA3QfaNQTXLzLwETQ/rapioca-recheada-com-morango.jpg',
      altImg: 'Imagem de uma tapioca',
      link: 'https://receitas.globo.com/blog/alimentacao-e-saude/tapioca-engorda-conheca-mitos-e-verdades-do-alimento.ghtml',
    },
  ],
};

export const MOCK_CULINARY: CarouselData = {
  card: [
    {
      title:
        'Chef Erick Jacquin fala o que não devia e expõe preferência por outro programa: "Muito melhor".',
      description:
        'Erick Jacquin, um dos jurados do MasterChef Brasil, revelou quais são seus programas favoritos na televisão. Em entrevista, o cozinheiro disse que sente falta de ter um espaço próprio na telinha.',
      srcImg:
        'https://metropolitanafm.com.br/wp-content/uploads/2023/05/img_chef_jacquin_2023-600x338.jpg',
      altImg: 'Foto de Erick Jacquin',
      link: 'https://metropolitanafm.com.br/televisao/chef-erick-jacquin-fala-o-que-nao-devia-e-expoe-preferencia-por-outro-programa-muito-melhor',
    },
    {
      title:
        'Sabores do Brasil: culinária brasileira é um dos principais impulsionadores do turismo.',
      description:
        'Quatro cidades brasileiras integram a Rede Brasileira de Cidades Criativas da Unesco na categoria gastronômica e revelam ser um destaque em combinação de sabores únicos do país.',
      srcImg:
        'https://www.gov.br/turismo/pt-br/assuntos/noticias/sabores-do-brasil-culinaria-brasileira-e-um-dos-principais-impulsionadores-do-turismo/PortalMTur.png/@@images/2f30bc58-6dbb-4a38-8d06-8dc2ced1dc78.png',
      altImg: 'Foto de comidas brasileiras',
      link: 'https://www.gov.br/turismo/pt-br/assuntos/noticias/sabores-do-brasil-culinaria-brasileira-e-um-dos-principais-impulsionadores-do-turismo',
    },
    {
      title: 'Band renova produção do MasterChef para 2025; saiba detalhes.',
      description:
        'A Band já acertou com a Endemol Shine Brasil a produção de novas temporadas do MasterChef, um de seus principais programas, para 2025. Inicialmente, a edição de amadores está confirmada.',
      srcImg:
        'https://www.otempo.com.br/adobe/dynamicmedia/deliver/dm-aid--1e7db789-e61b-4fcb-957b-9034cc0878d2/entretenimento-henrique-foga-a--helena-rizzo--ana-paula-padr-o-e-erick-jacquin-1716398874.jpg?quality=90&width=1200&preferwebp=true',
      altImg: 'Imagem dos jurados do Master Chef',
      link: 'https://www.otempo.com.br/entretenimento/2024/8/20/band-renova-producao-do--masterchef--para-2025--saiba-detalhes',
    },
    {
      title: 'Tapioca engorda? Conheça mitos e verdades do alimento.',
      description:
        'A tapioca ganhou destaque em muitas receitas e virou parte do cardápio de quem busca uma alimentação mais saudável. Feita da massa da mandioca, ela também é consumida por quem pratica exercícios físicos, já que funciona como pré e pós-treino.',
      srcImg:
        'https://s2-receitas.glbimg.com/LNr5zJu-Fo1pY5TZjk2iCEOS2PE=/0x0:5184x3456/1000x0/smart/filters:strip_icc()/i.s3.glbimg.com/v1/AUTH_1f540e0b94d8437dbbc39d567a1dee68/internal_photos/bs/2022/B/J/TwmuA3QfaNQTXLzLwETQ/rapioca-recheada-com-morango.jpg',
      altImg: 'Imagem de uma tapioca',
      link: 'https://receitas.globo.com/blog/alimentacao-e-saude/tapioca-engorda-conheca-mitos-e-verdades-do-alimento.ghtml',
    },
    {
      title: 'Aprenda como fazer bolo de cenoura de liquidificador.',
      description:
        'O bolo de cenoura é uma opção simples e prática para o café da manhã, lanche da tarde ou para uma ocasião especial com famílias e amigos. Essa receita é feita no liquidificador e fica pronta em menos de 1 hora.',
      srcImg:
        'https://s2-receitas.glbimg.com/CfHtIpRubb2sm1XzO65Vwp711HA=/0x0:1280x800/1000x0/smart/filters:strip_icc()/i.s3.glbimg.com/v1/AUTH_1f540e0b94d8437dbbc39d567a1dee68/internal_photos/bs/2021/9/N/bmnJNgR5G0K2QmEYFkPQ/bolo-de-cenoura-receita.jpg',
      altImg: 'Imagem de um bolo de cenoura',
      link: 'https://receitas.globo.com/tipos-de-prato/bolos/bolo-de-cenoura-de-liquidificador-4e80cb6a8811965be7003c43.ghtml',
    },
    {
      title:
        "Inspirada em 'Barbie', doceria Pikurrucha's, em SP, anuncia doces e drinques cor-de-rosa.",
      description:
        'Pioneira no conceito do espaço instagramável, a confeitaria decidiu ampliar o ambiente cor-de-rosa que dialoga com a boneca para seu cardápio, oferecendo opções como um café rosa com brigadeiro branco e pipoca doce.',
      srcImg:
        'https://f.i.uol.com.br/fotografia/2023/07/14/168935808464b18f049419d_1689358084_3x2_lg.jpg',
      altImg: "Imagem da doceria Pikurrucha's",
      link: 'https://guia.folha.uol.com.br/restaurantes/2023/07/inspirada-em-barbie-doceria-pikurruchas-em-sp-anuncia-doces-e-drinques-cor-de-rosa.shtml',
    },
    {
      title: 'Receitas de Chocolate Quente.',
      description:
        'Se você nunca fez um chocolate quente caprichado, vamos começar pela receita básica que é bastante simples e requer apenas alguns poucos ingredientes, mas que fica muito boa.',
      srcImg:
        'https://panelaterapia.com/wp-content/uploads/2023/07/chocolate2.png',
      altImg: 'Imagem de um chocolate quente',
      link: 'https://panelaterapia.com/2023/07/chocolate-quente-2.html',
    },
    {
      title: 'Pudim de leite condensado.',
      description:
        'Veja como fazer o pudim de leite condensado tradicional, que leva apenas 4 ingredientes: açúcar, leite condensado, leite e ovos. Ana Maria Braga ensina o passo a passo desta sobremesa tipicamente brasileira que vai encantar sua família.',
      srcImg:
        'https://s2-receitas.glbimg.com/z2JiB6nwO_XITDhQksL2uiAhic0=/0x0:984x765/1000x0/smart/filters:strip_icc()/i.s3.glbimg.com/v1/AUTH_1f540e0b94d8437dbbc39d567a1dee68/internal_photos/bs/2020/D/m/r8XAmWTcuRAzTRMBJZ1A/pudim-de-leite-condensado.jpg',
      altImg: 'Imagem de um pudim de leite condensado',
      link: 'https://receitas.globo.com/receitas-da-tv/mais-voce/pudim-de-leite-condensado-4d514bb052e0b252bc00e85a.ghtml',
    },
    {
      title:
        'A arte e o sabor das cachaças brasileiras que conquistam o Brasil e o mundo.',
      description:
        'No Dia Nacional de uma das bebidas mais populares no Brasil, conheça produtos nacionais premiados e reconhecidos.',
      srcImg:
        'https://www.gov.br/turismo/pt-br/assuntos/noticias/a-arte-e-o-sabor-das-cachacas-brasileiras-que-conquistam-o-brasil-e-o-mundo/PortalMTur1.png/@@images/0c711d59-9dc5-44cf-8b03-6689da228745.png',
      altImg: 'Imagem de bebidas alcoólicas brasileras',
      link: 'https://www.gov.br/turismo/pt-br/assuntos/noticias/a-arte-e-o-sabor-das-cachacas-brasileiras-que-conquistam-o-brasil-e-o-mundo',
    },
  ],
};

export const MOCK_TECHNOLOGY: CarouselData = {
  card: [
    {
      title:
        'X (Twitter) anuncia encerramento das operações no Brasil, mas seguirá disponível aos usuários.',
      description:
        'O X (Twitter) comunicou o fim das suas atividades oficiais no Brasil. A decisão vem após a rede social de Elon Musk se negar a cumprir uma decisão judicial do Supremo Tribunal Federal (STF).',
      srcImg: 'https://t2.tudocdn.net/677192?w=646&h=284',
      altImg: 'Logo do Twitter X',
      link: 'https://www.tudocelular.com/android/noticias/n225049/x-twitter-anuncia-fim-operacoes-no-brasil.html',
    },

    {
      title:
        'Bard, o "ChatGPT do Google", é lançado no Brasil; veja como funciona.',
      description:
        'O Bard, ferramenta de inteligência artificial do Google, foi lançado nesta quinta-feira (13) no Brasil. O serviço, que funciona como o ChatGPT, é gratuito e chega ao país pouco mais de três meses após ser lançado nos Estados Unidos e no Reino Unido.',
      srcImg:
        'https://googlediscovery.com/wp-content/uploads/bard-2-800x500.jpg',
      altImg: 'Logo do Google Bard',
      link: 'https://g1.globo.com/tecnologia/noticia/2023/07/13/bard-o-chatgpt-do-google-e-lancado-no-brasil-veja-como-funciona.ghtml',
    },

    {
      title:
        'Discord: O que é e como funciona o app para gamers que virou foco de abusos contra jovens.',
      description:
        'Plataforma ganhou popularidade entre os jovens por transmissões ao vivo de vídeos das partidas, mas virou mecanismo da ação de criminosos para subornar menores.',
      srcImg:
        'https://t.ctcdn.com.br/OBqzxQ888c3zUJEzGrxu4jjioOo=/512x288/smart/filters:format(webp)/i525670.png',
      altImg: 'Logo do aplicativo Discord',
      link: 'https://www.estadao.com.br/link/discord-o-que-e-e-como-funciona-o-app-para-gamers-que-virou-foco-de-abusos-contra-jovens/',
    },

    {
      title:
        'Samsung desafia lealdade ao iPhone com paródia de terror em campanha do Galaxy Z Flip 5.',
      description:
        '"Join the Flip Side" brinca com quem diz que "jamais trocaria meu smartphone".',
      srcImg:
        'https://www.sammobile.com/wp-content/uploads/2023/07/Galaxy-Z-Flip-Join-the-Flip-Side.jpg',
      altImg: 'Imagem novo smartphone Samsung',
      link: 'https://www.b9.com.br/163580/samsung-desafia-lealdade-ao-iphone-com-parodia-de-terror-em-campanha-do-galaxy-z-flip-5/',
    },

    {
      title:
        'Marvel é criticada por usar inteligência artificial para criar abertura de série.',
      description:
        'A controvérsia surgiu após a revelação de que a Marvel usou IA para criar a sequência de abertura de sua nova série, “Invasão Secreta”.',
      srcImg:
        'https://www.hardware.com.br/wp-content/uploads/2023/06/WhatsApp-Image-2023-06-22-at-11.11.20.jpeg',
      altImg: 'Imagem da nova série Marvel: Invasão Secreta',
      link: 'https://www.hardware.com.br/noticias/2023-06-marvel-e-criticada-por-usar-inteligencia-artificial-para-criar-abertura-de-serie.html',
    },

    {
      title:
        'Para ficar com Activision, Microsoft admite que Xbox perdeu guerra dos consoles.',
      description:
        'Em um documento enviado à corte, a Microsoft revela que, desde que foi lançado, em 2001, “o console Xbox ficou consistentemente com a terceira posição entre as três concorrentes, atrás de PlayStation e Nintendo nas vendas”.',
      srcImg:
        'https://files.tecnoblog.net/wp-content/uploads/2022/04/xbox-x_capa-2_tb-1060x596.png',
      altImg: 'Imagem do console Microsoft Xbox',
      link: 'https://tecnoblog.net/noticias/2023/06/22/para-ficar-com-activision-microsoft-admite-que-xbox-perdeu-guerra-dos-consoles/',
    },

    {
      title: 'VLibras - Tradução automática para tornar a Web mais acessível.',
      description:
        'As aplicações da Suíte VLibras permitem que pessoas surdas acessem conteúdo multimídia em sua língua natural de comunicação, o que contribui para com a acessibilidade de computadores, dispositivos móveis e páginas Web.',
      srcImg: 'https://www.gov.br/governodigital/pt-br/acessibilidade-e-usuario/vlibras/imagens/avatar.png',
      altImg: 'Avatares do VLibras: Guga, Ícaro e Hozana',
      link: 'https://www.gov.br/governodigital/pt-br/vlibras/',
    },

    {
      title:
        'Sony vai investir US$ 2,2 bilhões em pesquisa e desenvolvimento na unidade de games.',
      description:
        'A rival Microsoft recorreu a aquisições, como a operação com a Activision Blizzard, para fazer frente à Sony, que segue como líder do setor.',
      srcImg:
        'https://www.infomoney.com.br/wp-content/uploads/2023/07/GettyImages-1438610077.jpg?w=594&quality=70&strip=all',
      altImg: 'Imagem do controle de PS5',
      link: 'https://www.infomoney.com.br/negocios/sony-vai-investir-us-22-bilhoes-em-pesquisa-e-desenvolvimento-na-unidade-de-games/',
    },

    {
      title:
        'Instagram lança Threads, rede social para rivalizar com o Twitter.',
      description:
        'Rede social foi revelada dias após o Twitter anunciar um limite para leitura de posts. Usuários poderão fazer login e encontrar amigos por meio de suas contas no Instagram.',
      srcImg:
        'https://s2-g1.glbimg.com/XvJgv46mktFspeaOXTWubWa87Mg=/0x0:6000x3375/1008x0/smart/filters:strip_icc()/i.s3.glbimg.com/v1/AUTH_59edd422c0c84a879bd37670ae4f538a/internal_photos/bs/2023/y/6/jynisCQqywFngyjBH5Jg/aplicativo-threads.png',
      altImg: 'Imagens da nova rede social: "Threads"',
      link: 'https://g1.globo.com/tecnologia/noticia/2023/07/05/instagram-threads-rival-twitter.ghtml',
    },

    {
      title: 'PlayStation 2 completa 23 anos: relembre fatos e curiosidades.',
      description:
        'Com um design icônico e jogos inesquecíveis, como GTA San Andreas, Final Fantasy X e Metal Gear Solid 2, o PlayStation 2 tornou-se um dos consoles mais populares de todos os tempos.',
      srcImg:
        'https://www.oficinadanet.com.br/media/post/45086/750/playstation-2.jpg',
      altImg: 'Imagem do console PlayStation 2',
      link: 'https://www.oficinadanet.com.br/playstation/45086-playstation-2-fatos-curiosidades',
    },

    {
      title:
        'O que é o WikiLeaks? Saiba mais sobre o site criado por Julian Assange.',
      description:
        'O WikiLeaks é uma organização que divulga na internet documentos confidenciais obtidos de empresas e agências governamentais do mundo todo. Segundo o ativista, os processos aos quais ele responde têm a ver, principalmente, com o papel do WikiLeaks na divulgação de arquivos secretos de países e corporações.',
      srcImg:
        'https://proxy.olhardigital.com.br/wp-content/uploads/2019/04/20190411065012.jpg',
      altImg: 'Imagem de Julian Assange - Criador do WikiLeaks',
      link: 'https://olhardigital.com.br/2019/04/11/noticias/o-que-e-o-wikileaks-saiba-mais-sobre-o-site-criado-por-julian-assange/',
    },

    {
      title:
        'Entenda falha no sistema da CrowdStrike que causou apagão cibernético.',
      description:
        'Uma falha na atualização de conteúdo relacionada ao sensor de segurança CrowdStrike Falcon, que serve para detectar possíveis invasões de hackers, foi a causa do ataque cibernético desta sexta-feira (19), que deixou milhares de empresas e pessoas em todo o mundo sem acesso a sistemas operacionais, especialmente o Windows, da Microsoft.',
      srcImg:
        'https://imagens.ebc.com.br/JuyJu4pOSOAky0SReX3dQQGkWw4=/1170x700/smart/https://agenciabrasil.ebc.com.br/sites/default/files/thumbnails/image/2022-05-06t162338z_1_lynxnpei450rz_rtroptp_4_data-global-bis.jpg?itok=OTPN2eAC',
      altImg: 'Teclado de computador iluminado por código cibernético',
      link: 'https://agenciabrasil.ebc.com.br/geral/noticia/2024-07/entenda-falha-no-sistema-da-crowdstrike-que-causou-apagao-cibernetico',
    },

    {
      title: 'Capacitação Gratuita em Introdução à Programação.',
      description:
        'O Portal Tech é um programa criado pela AWS, em duas fases, para abrir um portal de acesso ao conhecimento sobre algumas das tecnologias que mais crescem no mundo: a computação em nuvem, o metaverso e o marketing digital.',
      srcImg:
        'https://s3.sa-east-1.amazonaws.com/proz-joy-class/production/instances/12de7cca531c4dbdb6e42c937f21c1a81703272239039.png',
      altImg: 'Logo Portal Tech - Proz, Amazon e Meta',
      link: 'https://pages.prozeducacao.com.br/proz-tecnologia',
    },
  ],
};
